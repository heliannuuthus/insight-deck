...
__embedding_model = HuggingFaceEmbeddings(
...
def compute_article_score(article: Result) -> Tuple[Result, Score]:
    """
    对单篇文章打分，返回总分和各维度的分数
    ...
    return article, Score(average_score=sum(scores.values()) / len(scores),
                          scores=scores)
